		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>Produkt znajduje się już w twoim koszyku!</b>
				</div>
			";
		} else {
			...
			if(mysqli_query($con,$sql)){
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Produkt został dodany do koszyka!</b>
					</div>
				";
			}
		}
				...
				echo '
                    <div class="product-widget">
												...
												<div class="product-body">
													...
													<h4 class="product-price"><span class="qty">'.$n.'</span>'.$product_price.' Zł</h4>
												</div>
											</div>'
                    ;
			}
            echo '<div class="cart-summary">
				    <small class="qty">'.$n.' Produkt\y wybrane</small>
				    <h5>'.$total_price.' Zł</h5>
				</div>'
            ?>
			<?php
			exit();
		...
		if (mysqli_num_rows($query) > 0) {
			...
			<form method="post" action="login_form.php">
	               <table id="cart" class="table table-hover table-condensed" id="">
    				<thead>
						<tr>
							<th style="width:50%">Produkt/y</th>
							<th style="width:10%">Cena</th>
							<th style="width:8%">Ilość</th>
							<th style="width:7%" class="text-center">Cena</th>
				...
				<tfoot>
					...
					<tr>
						<td><a href="store.php" class="btn btn-warning"><i class="fa fa-angle-left"></i> Kontynuj Zakupy</a></td>
				...
				if (!isset($_SESSION["uid"])) {
					echo '
							<a href="" data-toggle="modal" data-target="#Modal_register" class="btn btn-success">Gotowy do zapłaty</a></td>
								</tr>
							</tfoot>
							</table></div></div>';
					...
					echo '
						...
						<form action="checkout.php" method="post">
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/Anime_shop/Sklep/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/Anime_shop/Sklep/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/Anime_shop/Sklep/cancel.php"/>
									<input type="hidden" name="currency_code" value="USD"/>
									<input type="hidden" name="custom" value="'.$_SESSION["uid"].'"/>
									<input type="submit" id="submit" name="login_user_with_product" name="submit" class="btn btn-success" value="Gotowe do zapłaty">
									</form></td>
									</tr>
									</tfoot>	
							</table></div></div>    
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if(mysqli_query($con,$sql)){
		echo "<div class='alert alert-danger'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Produkt został usunięty z koszyka</b>
				</div>";
		exit();
	}
}
