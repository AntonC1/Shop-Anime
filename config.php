...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($username)) { array_push($errors, "Nazwa Użytkownika wymagana"); }
  if (empty($email)) { array_push($errors, "Email wymagane"); }
  if (empty($password_1)) { array_push($errors, "Hasło wymagane"); }
  if ($password_1 != $password_2) {
	array_push($errors, "Te dwa hasła nie są takie same");
  }
  ...
  if ($user) {
    ...
    if ($user['email'] === $email) {
      array_push($errors, "Email jest już przypisany do innego konta");
    }
  }
  if (count($errors) == 0) {
  ...
  if (count($errors) == 0) {
  	...
  	if (mysqli_num_rows($results) == 1) {
  	  ...
  	  header('location: index.php');
  		...
  		array_push($errors, "Nieprawidłowa kombinacja nazwy użytkownika/hasła");
  	}
  }
}
?>